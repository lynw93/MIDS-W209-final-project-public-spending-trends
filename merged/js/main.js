...
let selectedVal = "2023"; // Selected value from selector
...
function updateVisualizations() {
    ...
    if (selectedView === "totalSpending" && typeof renderTotalSpendingChart === "function") {
        renderTotalSpendingChart(selectedYear);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const totalSelector = document.getElementById('total_selector');
    const budgetSelector = document.getElementById('budget_selector');

    if (totalSelector) {
        totalSelector.addEventListener('change', (event) => {
            selectedVal = event.target.value;
            document.querySelectorAll('.selected-category-display').forEach(el => el.textContent = selectedVal);

            // Only update if totalSpending is active
    ...
    if (budgetSelector) {
        budgetSelector.addEventListener('change', (event) => {
            selectedVal = event.target.value;
            document.querySelectorAll('.selected-year-display').forEach(el => el.textContent = selectedVal);

            // Only update if budgetCategories is active
