...
async function renderSpendingChangeChart(beginYear, endYear) {
    ...
    const color = d3.scaleSequential()
        .domain(d3.extent(changes, d => d.percent))
    ...
    .attr("y", d => {
        const barHeight = Math.abs(y(d.change) - y(0));
        // Adjust label position for small bars
        return d.change >= 0
            ? y(d.change) - (barHeight > 15 ? 5 : -10)
            : y(d.change) + (barHeight > 15 ? 15 : 20);
    })
