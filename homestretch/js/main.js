...
async function loadData() {
    try {
        ...
        console.error('Error loading the budget subfunction data:', error);
...
function processData() {
    ...
    Object.keys(processedYearlyFullData).forEach(year => {
        ...
        let percentages = [];
        let parents = [];
        let labelMap = {}; // Keep track of the actual label used for each category
        ...
        Object.keys(processedYearlyFullData[year]).filter(cat => cat !== 'total').forEach(cat => {
            ...
            const catLabel = cat; // ✅ use plain category name
            labelMap[cat] = catLabel;
            ...
            if (subCatData && Object.keys(subCatData).length > 0) {
                Object.keys(subCatData).forEach(subCat => {
                    const uniqueLabel = `${subCat} (${cat})`; // disambiguate same-named children
                    categories.push(uniqueLabel);
                    values.push(subCatData[subCat]);
                    percentages.push((subCatData[subCat] / catAmount * 100).toFixed(1));
                    parents.push(labelMap[cat]); // ✅ reference the exact label used for parent
                });
            } else {
                console.warn(`No subfunctions for ${cat} in ${year}`);
            }
        });

        console.log(`Hierarchy for ${year}:`);
        console.table(categories.map((label, i) => ({
        label,
        parent: parents[i],
        value: values[i]
        })));
        ...
        console.log("=== Hierarchical Breakdown for", year, "===");
        console.table(categories.map((label, i) => ({
        label,
        parent: parents[i],
        value: values[i]
...
function initializeApplication() {
    ...
    if (beginYearSelector && endYearSelector && spendingIframe) {
        function postSpendingChangeMessage() {
            ...
            if (spendingIframe.contentWindow) {
