...
async function renderImpactfulChart(container, { data, selectedEvents }) {
    ...
    if (selectedEvents.covid) {
        ...
        selectedCategories.forEach(cat => {
            ...
            g.append("circle")
                ...
                .attr("r", 6)
                .attr("fill", "#900")
                .attr("opacity", highlightedCategory && highlightedCategory !== cat ? 0.1 : 0.7)
    ...
    renderCategoryLinesAndLabels(g, categories, selectedCategories, x, y, line, color);
    ...
    function renderCategoryLinesAndLabels(g, categories, selectedCategories, x, y, line, color) {
        selectedCategories.forEach(cat => {
            const data = categories[cat];
            const last = data[data.length - 1];
        
            // Line with click behavior
            g.append("path")
                .datum(data)
                .attr("class", "category-line")
                .attr("fill", "none")
                .attr("stroke", color(cat))
                .attr("stroke-width", highlightedCategory === cat ? 4 : 2)
                .style("opacity", highlightedCategory && highlightedCategory !== cat ? 0.15 : 1)
                .attr("d", line)
                .style("cursor", "pointer")
                .on("click", () => {
                    highlightedCategory = (highlightedCategory === cat) ? null : cat;
                    updateImpactfulEventsVisualization(); // Redraw
                });
        
            // Label with same logic
            g.append("text")
